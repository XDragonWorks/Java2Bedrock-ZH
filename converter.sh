...
then 
  ...
  status_message process "正在为基岩版资源包生成初始目录结构"
...
status_message process "下载最新的Geyser物品映射(加速地址)"
mkdir -p ./scratch_files
printf "\e[3m\e[37m"
echo
COLUMNS=$COLUMNS-1 curl --no-styled-output -#L -o scratch_files/item_mappings.json https://mirror.ghproxy.com/https://raw.githubusercontent.com/GeyserMC/mappings/master/items.json
echo
COLUMNS=$COLUMNS-1 curl --no-styled-output -#L -o scratch_files/item_texture.json https://mirror.ghproxy.com/https://raw.githubusercontent.com/Kas-tle/java2bedrockMappings/main/item_texture.json
echo
printf "${C_CLOSE}"
...
do
   convert_model () {
    ...
    status_message process "开始以 GeyserID ${gid} 转换模型"
    ...
    def element_array:
      ...
      status_message completion "${gid} 转换完成\n$(ProgressBar ${tot_pos} ${_end})"
      echo
   }
   wait_for_jobs
